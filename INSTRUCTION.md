...
| `Address already in use` / port 5000 busy | Another program is using port 5000 | Stop it, or change `PORT=5000` to `PORT=5050` in `.env`, then visit `http://127.0.0.1:5050` |
...
running you can open `http://<your-computer's-local-IP>:5000` from a phone
